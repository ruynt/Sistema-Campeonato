...
function InscricoesAdminPage() {
  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      ...
      {resumo ? (
        <section className="card" aria-label="Resumo de inscrições">
                  ...
                  {inscricoesIndividuais.length ? (
                    inscricoesIndividuais.map((i: any) => (
                      <tr key={`individual-${i.id}`}>
                        <td data-label="Tipo">Individual</td>
                        <td data-label="Jogador">
                          <strong>{i.usuario?.nome || "—"}</strong>
                          {i.usuario?.email ? ` — ${i.usuario.email}` : ""}
                        </td>
                        <td data-label="Status">{traduzirStatusAnalise(i.statusAnalise)}</td>
                        <td data-label="Camisa">{i.tamanhoCamisa || "—"}</td>
                        <td data-label="Valor">
                          {formatarDinheiroCentavos(i.valorTotalCentavos)}
                        </td>
                          ...
                          <div style={{ display: "inline-flex", gap: 8, flexWrap: "wrap" }}>
                            <button
                              ...
                              disabled={
                                ...
                                String(i.status || "").toUpperCase() === "USADA_EM_EQUIPE"
                              }
                              title="Aprovar"
                              aria-label="Aprovar"
                            >
                    ...
                    <tr>
                      <td colSpan={6} className="campeonatos-empty">
